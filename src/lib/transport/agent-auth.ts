...
export const ARCADE555_AGENT_API_KEY_ENV = "ARCADE555_AGENT_API_KEY";
export const ARCADE555_AGENT_TOKEN_ENV = "ARCADE555_AGENT_TOKEN";
...
async function exchangeTokenWithApiKey(baseUrl: string, apiKey: string): Promise<string> {
  ...
  inFlightExchange = (async () => {
    ...
    const response = await runtimeConfig.fetchImpl(exchangeUrl.toString(), {
      method: "POST",
      headers: {
...
export function isAgentAuthConfigured(): boolean {
  return Boolean(
    trimEnv(ARCADE555_AGENT_API_KEY_ENV) ||
      trimEnv(STREAM555_AGENT_API_KEY_ENV) ||
      trimEnv(ARCADE555_AGENT_TOKEN_ENV) ||
...
export function describeAgentAuthSource(): string {
  if (trimEnv(ARCADE555_AGENT_API_KEY_ENV)) {
    return `${ARCADE555_AGENT_API_KEY_ENV} (short-lived JWT exchange)`;
  }
  ...
  if (
    ...
    return `${ARCADE555_AGENT_TOKEN_ENV}|${STREAM555_AGENT_TOKEN_ENV}|${STREAM_API_BEARER_TOKEN_ENV} (static bearer)`;
  }
...
export async function resolveAgentBearer(baseUrl: string): Promise<string> {
  const apiKey =
    trimEnv(ARCADE555_AGENT_API_KEY_ENV) || trimEnv(STREAM555_AGENT_API_KEY_ENV);
  if (apiKey) {
  ...
  throw new Error(
    `${ARCADE555_AGENT_API_KEY_ENV} or ${ARCADE555_AGENT_TOKEN_ENV} (or ${STREAM555_AGENT_API_KEY_ENV}|${STREAM555_AGENT_TOKEN_ENV}|${STREAM_API_BEARER_TOKEN_ENV}) is required`,
  );
}

...
import { resolveArcade555StateDir } from "../milaidy/runtime.js";
import type {
...
const ROOT_DIR = path.join("arcade555", "mastery");
const LEGACY_ROOT_DIR = "five55-mastery";
const RUNS_DIR = "runs";
...
function stateRootDir(): string {
  return resolveArcade555StateDir(process.env);
}

function masteryRootDir(): string {
  return path.join(stateRootDir(), ROOT_DIR);
}

function legacyMasteryRootDir(): string {
  return path.join(stateRootDir(), LEGACY_ROOT_DIR);
}
...
async function ensureDirs(): Promise<void> {
  await ensureCanonicalRootReady();
  await fs.mkdir(path.join(masteryRootDir(), RUNS_DIR), { recursive: true });
}

let rootReadyPromise: Promise<void> | null = null;
...
async function ensureCanonicalRootReady(): Promise<void> {
  if (!rootReadyPromise) {
    rootReadyPromise = (async () => {
      const canonicalRoot = masteryRootDir();
      if (await pathExists(canonicalRoot)) {
        return;
      }

      const legacyRoot = legacyMasteryRootDir();
      if (await pathExists(legacyRoot)) {
        await fs.mkdir(path.dirname(canonicalRoot), { recursive: true });
        await fs.cp(legacyRoot, canonicalRoot, {
          recursive: true,
          force: false,
          errorOnExist: false,
        });
        await writeJsonAtomic(path.join(canonicalRoot, MIGRATION_MARKER_FILE), {
      ...
      await fs.mkdir(path.join(canonicalRoot, RUNS_DIR), { recursive: true });
    })();
  }

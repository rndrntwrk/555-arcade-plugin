...
function nowIso(): string {
  return new Date(getArcade555RuntimeConfig().now()).toISOString();
}
...
function buildRunId(suiteId: string): string {
  return `${suiteId}-${new Date(getArcade555RuntimeConfig().now()).toISOString().replace(/[-:.]/g, "").replace("T", "T").slice(0, 15)}-${crypto.randomBytes(3).toString("hex")}`;
}

function pickSeed(seedMode: "fixed" | "mixed" | "rolling", offset: number): number {
  if (seedMode === "fixed") return 555;
  if (seedMode === "rolling") return getArcade555RuntimeConfig().now() + offset;
...
async function executePlan(
  ...
  const response = await getArcade555RuntimeConfig().fetchImpl(
    new URL("/api/agent/autonomy/execute-plan", baseUrl),
    {
      method: "POST",
      headers: {
...
const globalKey = "__milaidyArcade555MasteryOrchestrator";
const legacyGlobalKey = "__milaidyFive55MasteryOrchestrator";

type GlobalWithOrchestrator = typeof globalThis & {
  [globalKey]?: MasteryCertificationOrchestrator;
  [legacyGlobalKey]?: MasteryCertificationOrchestrator;
};
...
export function getMasteryCertificationOrchestrator(): MasteryCertificationOrchestrator {
  ...
  if (!g[globalKey] && g[legacyGlobalKey]) {
    g[globalKey] = g[legacyGlobalKey];
  }
  if (!g[globalKey]) {
